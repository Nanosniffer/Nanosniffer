...
type IntakeTab = 'evidence' | 'suspect' | 'comms' | 'financial' | 'vehicle';
...
export const EvidenceIntakePage: React.FC = () => {
  ...
  // Fetch recent evidence and criminals to display in live activity
  const { data: timelineRes } = useQuery({ queryKey: ['timeline'], queryFn: () => getTimelineEvents() });
  const { data: criminalsRes } = useQuery({ queryKey: ['criminals'], queryFn: () => getCriminals() });
  ...
  const [suspectForm, setSuspectForm] = useState({
    ...
    fingerprintId: '',
    heightCm: 182,
    tags: 'Under Active Surveillance, High Value Target',
  });
      ...
      if (activeTab === 'evidence') {
        setEvidenceForm((prev) => ({
          ...
          description: prev.description
            ? `${prev.description} [AI Flagged: High correlation with Vanguard Cyber Syndicate maritime logistics channel].`
            : 'AI cross-correlation identified direct temporal match with tapped satellite communication burst #ST-8819.',
        }));
        ...
        setSuspectForm((prev) => ({
          ...
          biography: prev.biography
            ...
            : 'Key facilitator identified across 4 active wiretap logs and offshore shell accounts in Panama.',
        }));
      }
    ...
    setTimeout(() => {
      if (activeTab === 'evidence') {
        const newEvent: TimelineEvent = {
          ...
          location: evidenceForm.location || 'Monitored Tactical Sector',
        ...
        addNotification({
          title: `Field Intelligence Node Logged (#${generatedId})`,
          message: `Intake verified by Officer ${user?.name || 'Vance'}.`,
          type: 'system',
          severity: 'HIGH',
        });
    ...
    <div className="space-y-5 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-lg border border-slate-200 shadow-card">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="px-2.5 py-1 rounded bg-slate-50 border border-slate-200 text-xs font-mono text-slate-600">
            Badge: <span className="font-semibold text-slate-900">{user?.badgeNumber || 'AGY-7701'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="p-3.5 space-y-1">
          <span className="text-[11px] font-semibold text-slate-500 flex items-center justify-between">
            Evidence Items Logged
            <FileCheck className="w-3.5 h-3.5 text-slate-400" />
          </span>
          <div className="text-xl font-bold text-slate-900">{timelineEvents.length}</div>
          <span className="text-[10px] text-emerald-600 font-medium">Chain of Custody Active</span>
        </Card>
        ...
        <Card className="p-3.5 space-y-1">
          ...
          <div className="text-xl font-bold text-slate-900">{criminals.length}</div>
          <span className="text-[10px] text-slate-500 font-medium">Interpol Roster</span>
        </Card>
        ...
        <Card className="p-3.5 space-y-1">
          ...
          <span className="text-[10px] text-blue-600 font-medium">Live Intercepts</span>
        </Card>
        ...
        <Card className="p-3.5 space-y-1">
          ...
          <div className="text-xl font-bold text-slate-900">$42.5M</div>
          <span className="text-[10px] text-amber-600 font-medium">AML Monitored</span>
        </Card>
      </div>

      {/* Main Two-Column Layout */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        {/* Left Column: Multi-Tab Intake Form (8 Cols) */}
        <div className="lg:col-span-8 space-y-4">
          <Card className="overflow-hidden border border-slate-200 shadow-card">
            {/* Tab Bar */}
            <div className="flex items-center gap-1 px-4 border-b border-slate-200 bg-slate-50 overflow-x-auto text-xs shrink-0 py-2">
              {[
                { id: 'evidence', label: '1. Physical / Forensic Evidence', icon: FileCheck },
                { id: 'suspect', label: '2. Suspect Profile', icon: User },
                ...
                <div className="p-6 rounded-lg bg-emerald-50 border border-emerald-200 text-center space-y-4 animate-in zoom-in-95">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 mt-0.5">
                      Entry integrated into live intelligence database, network relationship graph, and timeline.
                    </p>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 text-xs">
                    ...
                    <div className="space-y-3.5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            type="text"
                            required
                            placeholder="e.g. CCTV Sighting: Slip 42 Submersible Offload"
                      ...
                      <div>
                        ...
                        <textarea
                          rows={3}
                          required
                          placeholder="Enter raw observation notes, timestamps, serial numbers, or wiretap snippets..."
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 rounded-lg bg-slate-50 border border-slate-200">
                        <div>
                          ...
                          <div className="space-y-1 text-[11px] text-slate-600">
                            <div>Officer: <span className="font-semibold text-slate-800">{user?.name || 'Agent Marcus Vance'}</span></div>
                            <div>Badge: <span className="font-mono text-slate-800">{evidenceForm.officerBadge}</span></div>
                            <div>Station: <span className="text-slate-800">{evidenceForm.stationId}</span></div>
                          </div>
                        </div>
                    ...
                    <div className="space-y-3.5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            placeholder="e.g. Ghost / NullPointer"
                      ...
                      <div>
                        <label className="block font-semibold text-slate-700 mb-1">
                          Intelligence Summary & Operational Roles
                        </label>
                        <textarea
                          rows={2}
                          placeholder="Known operations, criminal associates, and syndicate links..."
                    ...
                    <div className="space-y-3.5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            placeholder="e.g. Encrypted Satellite VoIP"
                      ...
                      <div>
                        ...
                        <textarea
                          rows={3}
                          placeholder='SUBJECT: "...consignment delivery confirmed for Slip 14 at 0200..."'
                  ...
                  <div className="pt-3 border-t border-slate-200 flex items-center justify-end gap-2">
                    <Button
                      ...
                      className="text-xs gap-1.5 bg-slate-900 hover:bg-slate-800 text-white shadow-sm font-semibold"
                    >
                      <PlusCircle className="w-3.5 h-3.5 text-brand-300" />
                      <span>{isSubmitting ? 'Logging to Vault...' : 'Submit & Ingest to Vault'}</span>
                    </Button>
                  </div>
                </form>
              )}
        ...
        {/* Right Column: Live Recent Ingest Feed (4 Cols) */}
        <div className="lg:col-span-4 space-y-4">
          <Card className="p-4 space-y-3 border border-slate-200 shadow-card">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <h3 className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                <Activity className="w-3.5 h-3.5 text-brand-600" />
                Live Ingest Stream ({timelineEvents.length})
              </h3>
              <span className="text-[10px] font-mono text-emerald-600 bg-emerald-50 px-1.5 py-0.2 rounded border border-emerald-200 font-semibold">
                SYNCED
              </span>
            </div>
            ...
            <div className="space-y-2.5 max-h-[520px] overflow-y-auto pr-1">
              {timelineEvents.slice(0, 6).map((item) => (
                <div
                  key={item.id}
                  className="p-2.5 rounded-md bg-slate-50 border border-slate-200 text-xs space-y-1 hover:bg-slate-100/80 transition"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-900 truncate max-w-[170px]">{item.title}</span>
